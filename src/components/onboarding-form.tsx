...
import { useForm, Controller, useFieldArray, Control, FieldErrors, UseFormWatch } from "react-hook-form";
...
const PreferenceItem = ({
  ...
  return (
    <div className="space-y-2 p-3 border rounded-md">
      <Controller name={`${name}.${index}.name`} control={control} render={({ field }) => (
          <Input {...field} placeholder={placeholderName} className="text-base" />
        )}
      />
      <div className="flex items-center space-x-2">
        <Controller name={`${name}.${index}.addExplanation`} control={control} render={({ field }) => (
          <Checkbox id={`${name}.${index}.addExplanation`} checked={field.value} onCheckedChange={field.onChange} />
        )} />
        <label htmlFor={`${name}.${index}.addExplanation`} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          Add explanation
        </label>
      </div>
      {showExplanation && (
        <Controller name={`${name}.${index}.explanation`} control={control} render={({ field }) => (
...
const Step1 = ({ control, errors, watch }: { control: Control<OnboardingFormData>, errors: FieldErrors<OnboardingFormData>, watch: UseFormWatch<OnboardingFormData> }) => {
      ...
      <CardContent className="space-y-6">
        <FormItem>
          ...
          <Controller name="firstName" control={control} render={({ field }) => (
            <Input id="firstName" placeholder="Your first name" {...field} className="text-base"/>
          )} />
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <FormItem>
                ...
                <Controller name="age" control={control} render={({ field }) => (
                    <Input id="age" type="number" placeholder="e.g., 25" {...field} onChange={e => field.onChange(parseInt(e.target.value, 10))} className="text-base"/>
                )} />
        ...
        <FormItem>
            ...
            <Controller name="photo" control={control} render={({ field: { onChange, ...rest } }) => (
                <>
                <div className="relative flex items-center justify-center w-full">
                    <label htmlFor="dropzone-file" className={`flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer bg-secondary hover:bg-accent transition-colors ${errors.photo ? 'border-destructive' : 'border-border'}`}>
                        {previewUrl ? (
                ...
                {errors.photo && <FormMessage>{errors.photo.message?.toString()}</FormMessage>}
                </>
            )} />
        </FormItem>
      </CardContent>
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-4">
            {environments.map((item, index) => (
              <div key={item.id}>
                <Label htmlFor={`environments.${index}.explanation`} className="text-base font-semibold">Environment {index + 1}</Label>
                <Controller name={`environments.${index}.explanation`} control={control} render={({ field }) => (
...
export function OnboardingForm({ inviteCode }: OnboardingFormProps) {
  ...
  return (
    <div>
      ...
      <form ref={formRef} action={formAction} onSubmit={handleSubmit(() => formAction(new FormData(formRef.current!)))}>
        <input type="hidden" name="inviteCode" value={inviteCode} />

        <div className={currentStep === 1 ? 'block' : 'hidden'}><Step1 control={control} errors={errors} watch={watch} /></div>
